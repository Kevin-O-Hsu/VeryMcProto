...
/**
 * 工具集（移植自 {@code ch.endte.syncmatica.util.SyncmaticaUtil}）。
 *
 * <p>含文件名清理、原子写（{@link #backupAndReplace}）、MD5→UUID 校验和（{@link #createChecksum}，
 * 客户端会校验，不可改算法）、NBT 读取、{@code .litematic} peek（{@link #litematicPeek}）。
 *
 * <p><b>Paper 适配</b>：原版 {@code Syncmatica.LOGGER}（log4j）→ {@link SyncmaticaLog}（JUL shim）。
 */
public class SyncmaticaUtil
{
    ...
    private static boolean overwrite(final Path backup, final Path current, final int tries)
    {
        ...
        catch (final IOException exception)
        {
            if (tries <= 0)
            {
                SyncmaticaLog.error("Excessive retries when trying to write Syncmatica placement", exception);
    ...
    public static CompoundTag readNbtFromFile(@Nonnull Path file)
    {
        ...
        {
            SyncmaticaLog.warn("readNbtFromFile: Failed to read NBT data from file '{}'", file);
        }
