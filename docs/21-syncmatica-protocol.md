...
**本实现的关键决策**（`SyncmaticaReference.java:21-22`）：`MOD_VERSION = "1.0.0"`（非 `"0.1.x"`），使 `fromVersionString` 返回 null → **强制双方走 FEATURE 交换** → 双方用全集 FeatureSet（MODIFY/DISPLAY_NAME/CORE_EX/VERSION 全开）。这保证 metadata/position 编码所有可选字段都下发，功能完整。
