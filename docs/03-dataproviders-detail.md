...
| `TweaksDataProvider` | `servux:tweaks_data` | 1 | tweak 元数据 + NBT 查询（复用 Entities 逻辑） | 同上 | 低 |
| `StructureDataProvider` | `servux:structure_bounding_boxes` | **2** | 原版结构边界框 | **纯 NMS**（`ChunkAccess.getAllReferences`/`StructureStart.createTag`） | **高** |
| `LitematicsDataProvider` | `servux:litematic_data` | 1 | 投影投递/粘贴/批量实体 | NMS（投影系统见 [05](05-schematic-system.md)） | 高 |
...
> 原版：`dataproviders/TweaksDataProvider.java`（373 行）。协议通道 `servux:tweaks_data`，协议版本 1。
...
> 原版：`dataproviders/StructureDataProvider.java`（557 行）。协议通道 `servux:structure_bounding_boxes`，协议版本 **2**。
...
> 原版：`dataproviders/LitematicsDataProvider.java`（424 行）。协议通道 `servux:litematic_data`，协议版本 1。
